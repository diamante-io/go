...
func TestBuildChallengeTx(t *testing.T) {
	...
	{
		// 1 minute timebound
		tx, err := BuildChallengeTx(kp0.Seed(), kp0.Address(), "testwebauth.diamcircle.org", "testanchor.diamcircle.org", network.TestNetworkPassphrase, time.Minute)
		assert.NoError(t, err)
		...
		assert.Equal(t, xdr.String64("testanchor.diamcircle.org auth"), op.Body.ManageDataOp.DataName, "DataName should be 'testanchor.diamcircle.org auth'")
		...
		assert.Equal(t, "testwebauth.diamcircle.org", string(*webAuthOp.Body.ManageDataOp.DataValue), "DataValue should be 'testwebauth.diamcircle.org'")
	}

	{
		// 5 minutes timebound
		tx, err := BuildChallengeTx(kp0.Seed(), kp0.Address(), "testwebauth.diamcircle.org", "testanchor.diamcircle.org", network.TestNetworkPassphrase, time.Duration(5*time.Minute))
		assert.NoError(t, err)
		txeBase64, err := tx.Base64()
		assert.NoError(t, err)
		...
		assert.Equal(t, xdr.String64("testanchor.diamcircle.org auth"), op1.Body.ManageDataOp.DataName, "DataName should be 'testanchor.diamcircle.org auth'")
...
func TestPreAuthTransaction(t *testing.T) {
	...
	//encode the txFutureHash as a diamcircle HashTx signer key.
...
func TestHashXTransaction(t *testing.T) {
	// 256 bit preimage
	preimage := "this is a preimage for hashx transactions on the diamcircle network"

	preimageHash := sha256.Sum256([]byte(preimage))

	//encode preimageHash as a diamcircle HashX signer key
...
func TestReadChallengeTx_validSignedByServerAndClient(t *testing.T) {
	...
	tx64, err := tx.Base64()
	require.NoError(t, err)
	readTx, readClientAccountID, _, err := ReadChallengeTx(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"testanchor.diamcircle.org"})
...
func TestReadChallengeTx_invalidOperationNoSourceAccount(t *testing.T) {
	...
	tx64, err := tx.Base64()
	require.NoError(t, err)
	_, _, _, err = ReadChallengeTx(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"testanchor.diamcircle.org"})
...
func TestReadChallengeTx_acceptsV0AndV1Transactions(t *testing.T) {
	...
	tx, err := BuildChallengeTx(
		kp0.Seed(),
		kp0.Address(),
		"testwebauth.diamcircle.org",
		"testanchor.diamcircle.org",
		network.TestNetworkPassphrase,
		time.Hour,
	)
	assert.NoError(t, err)
	...
	for _, challenge := range []string{v1Challenge, v0Challenge} {
		parsedTx, clientAccountID, _, err := ReadChallengeTx(
			...
			network.TestNetworkPassphrase,
			"testwebauth.diamcircle.org",
			[]string{"testanchor.diamcircle.org"},
		)
		assert.NoError(t, err)
...
func TestReadChallengeTx_doesVerifyHomeDomainFailure(t *testing.T) {
	...
	tx64, err := tx.Base64()
	require.NoError(t, err)
	_, _, _, err = ReadChallengeTx(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"willfail"})
	assert.EqualError(t, err, "operation key does not match any homeDomains passed (key=\"testanchor.diamcircle.org auth\", homeDomains=[willfail])")
}
...
func TestReadChallengeTx_matchesHomeDomain(t *testing.T) {
	...
	tx64, err := tx.Base64()
	require.NoError(t, err)
	_, _, matchedHomeDomain, err := ReadChallengeTx(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"testanchor.diamcircle.org"})
	require.NoError(t, err)
	assert.Equal(t, matchedHomeDomain, "testanchor.diamcircle.org")
}
...
func TestReadChallengeTx_doesNotMatchHomeDomain(t *testing.T) {
	...
	tx64, err := tx.Base64()
	require.NoError(t, err)
	_, _, matchedHomeDomain, err := ReadChallengeTx(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"not", "going", "to", "match"})
	assert.Equal(t, matchedHomeDomain, "")
	assert.EqualError(t, err, "operation key does not match any homeDomains passed (key=\"testanchor.diamcircle.org auth\", homeDomains=[not going to match])")
}
...
func TestReadChallengeTx_invalidWebAuthDomain(t *testing.T) {
	...
	require.NoError(t, err)
	_, _, _, err = ReadChallengeTx(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"testanchor.diamcircle.org"})
	assert.EqualError(t, err, `web auth domain operation value is "testwebauth.example.org" but expect "testwebauth.diamcircle.org"`)
}
...
func TestVerifyChallengeTxThreshold_invalidServer(t *testing.T) {
	...
	signersFound, err := VerifyChallengeTxThreshold(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"testanchor.diamcircle.org"}, threshold, signerSummary)
...
func TestVerifyChallengeTxThreshold_invalidServerAndMultipleClientKeyNotMeetingThreshold(t *testing.T) {
	...
	_, err = VerifyChallengeTxThreshold(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"testanchor.diamcircle.org"}, threshold, signerSummary)
...
func TestVerifyChallengeTxThreshold_doesNotMatchHomeDomain(t *testing.T) {
	...
	_, err = VerifyChallengeTxThreshold(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"not", "going", "to", "match"}, threshold, signerSummary)
	assert.EqualError(t, err, "operation key does not match any homeDomains passed (key=\"testanchor.diamcircle.org auth\", homeDomains=[not going to match])")
}
...
func TestVerifyChallengeTxThreshold_doesVerifyHomeDomainFailure(t *testing.T) {
	...
	_, err = VerifyChallengeTxThreshold(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"testanchor.diamcircle.org"}, threshold, signerSummary)
	assert.EqualError(t, err, "operation key does not match any homeDomains passed (key=\"will fail auth\", homeDomains=[testanchor.diamcircle.org])")
}
...
func TestVerifyChallengeTxThreshold_doesVerifyHomeDomainSuccess(t *testing.T) {
	...
	signers, err := VerifyChallengeTxThreshold(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"testanchor.diamcircle.org"}, threshold, signerSummary)
	assert.Equal(t, nil, err)
...
func TestVerifyChallengeTxSigners_invalidServer(t *testing.T) {
	...
	signersFound, err := VerifyChallengeTxSigners(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"testanchor.diamcircle.org"}, clientKP.Address())
	assert.Empty(t, signersFound)
	assert.EqualError(t, err, "transaction not signed by "+serverKP.Address())
}
...
func TestVerifyChallengeTxSigners_validServerAndMultipleClientSigners(t *testing.T) {
	...
	signersFound, err := VerifyChallengeTxSigners(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"testanchor.diamcircle.org"}, clientKP.Address(), clientKP2.Address())
...
func TestVerifyChallengeTxSigners_validServerAndClientSignersNotMasterKey(t *testing.T) {
	...
	signersFound, err := VerifyChallengeTxSigners(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"testanchor.diamcircle.org"}, clientKP2.Address())
...
func TestVerifyChallengeTxSigners_validServerAndClientSignersIgnoresServerSigner(t *testing.T) {
	...
	signersFound, err := VerifyChallengeTxSigners(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"testanchor.diamcircle.org"}, serverKP.Address(), clientKP2.Address())
	assert.Equal(t, []string{clientKP2.Address()}, signersFound)
	assert.NoError(t, err)
}
...
func TestVerifyChallengeTxSigners_validServerAndClientSignersIgnoresDuplicateSigner(t *testing.T) {
	...
	signersFound, err := VerifyChallengeTxSigners(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"testanchor.diamcircle.org"}, clientKP2.Address(), clientKP2.Address())
	assert.Equal(t, []string{clientKP2.Address()}, signersFound)
	assert.NoError(t, err)
}
...
func TestVerifyChallengeTxSigners_validIgnorePreauthTxHashAndXHash(t *testing.T) {
	...
	signersFound, err := VerifyChallengeTxSigners(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"testanchor.diamcircle.org"}, clientKP2.Address(), preauthTxHash, xHash, unknownSignerType)
	assert.Equal(t, []string{clientKP2.Address()}, signersFound)
	assert.NoError(t, err)
}
...
func TestVerifyChallengeTxSigners_invalidServerAndClientSignersIgnoresDuplicateSignerInError(t *testing.T) {
	...
	signersFound, err := VerifyChallengeTxSigners(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"testanchor.diamcircle.org"}, clientKP.Address(), clientKP.Address())
	assert.Empty(t, signersFound)
	assert.EqualError(t, err, "transaction not signed by "+clientKP.Address())
}
...
func TestVerifyChallengeTxSigners_invalidServerAndClientSignersFailsSignerSeed(t *testing.T) {
	...
	signersFound, err := VerifyChallengeTxSigners(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"testanchor.diamcircle.org"}, clientKP2.Seed())
	assert.Empty(t, signersFound)
	assert.EqualError(t, err, "no verifiable signers provided, at least one G... address must be provided")
}
...
func TestVerifyChallengeTxSigners_invalidNoSigners(t *testing.T) {
	...
	_, err = VerifyChallengeTxSigners(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"testanchor.diamcircle.org"})
	assert.EqualError(t, err, "no verifiable signers provided, at least one G... address must be provided")
}
...
func TestVerifyChallengeTxSigners_doesVerifyHomeDomainFailure(t *testing.T) {
	...
	_, err = VerifyChallengeTxSigners(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"validation failed"}, clientKP.Address())
	assert.EqualError(t, err, "operation key does not match any homeDomains passed (key=\"testanchor.diamcircle.org auth\", homeDomains=[validation failed])")
}
...
func TestVerifyChallengeTxSigners_matchesHomeDomain(t *testing.T) {
	...
	_, err = VerifyChallengeTxSigners(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"testanchor.diamcircle.org"}, signers...)
	require.NoError(t, err)
}
...
func TestVerifyChallengeTxSigners_doesNotMatchHomeDomain(t *testing.T) {
	...
	_, err = VerifyChallengeTxSigners(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"not", "going", "to", "match"}, signers...)
	assert.EqualError(t, err, "operation key does not match any homeDomains passed (key=\"testanchor.diamcircle.org auth\", homeDomains=[not going to match])")
}
...
func TestVerifyChallengeTxSigners_doesVerifyHomeDomainSuccess(t *testing.T) {
	...
	_, err = VerifyChallengeTxSigners(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.diamcircle.org", []string{"testanchor.diamcircle.org"}, clientKP.Address())
	assert.Equal(t, nil, err)
}
