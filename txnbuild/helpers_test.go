...
func TestValidateDiamcirclePublicKey(t *testing.T) {
	validKey := "GDWZCOEQRODFCH6ISYQPWY67L3ULLWS5ISXYYL5GH43W7YFMTLB65PYM"
	err := validateDiamcirclePublicKey(validKey)
	assert.NoError(t, err, "public key should be valid")

	invalidKey := "GDWZCOEQRODFCH6ISYQPWY67L3ULLWS5ISXYYL5GH43W7Y"
	err = validateDiamcirclePublicKey(invalidKey)
	expectedErrMsg := "GDWZCOEQRODFCH6ISYQPWY67L3ULLWS5ISXYYL5GH43W7Y is not a valid diamcircle public key"
	...
	err = validateDiamcirclePublicKey(invalidKey)
	expectedErrMsg = "SBCVMMCBEDB64TVJZFYJOJAERZC4YVVUOE6SYR2Y76CBTENGUSGWRRVO is not a valid diamcircle public key"
	require.EqualError(t, err, expectedErrMsg, "public key should be invalid")
}

func TestValidateDiamcircleAssetWithValidAsset(t *testing.T) {
	nativeAsset := NativeAsset{}
	err := validateDiamcircleAsset(nativeAsset)
	assert.NoError(t, err)

	kp0 := newKeypair0()
	creditAsset := CreditAsset{"XYZ", kp0.Address()}
	err = validateDiamcircleAsset(creditAsset)
	assert.NoError(t, err)
}

func TestValidateDiamcircleAssetWithInValidAsset(t *testing.T) {
	err := validateDiamcircleAsset(nil)
	assert.Error(t, err)
	...
	err = validateDiamcircleAsset(creditAssetNoCode)
	assert.Error(t, err)
	...
	err = validateDiamcircleAsset(creditAssetNoIssuer)
	assert.Error(t, err)
...
func TestValidateAmountInvalidValue(t *testing.T) {
	...
	require.EqualError(t, err, expectedErrMsg, "should be a valid diamcircle amount")
