...
// CreateClaimableBalance represents the Diamcircle create claimable balance operation. See
...
func (cb *CreateClaimableBalance) Validate(withMuxedAccounts bool) error {
	for _, d := range cb.Destinations {
		err := validateDiamcirclePublicKey(d.Destination)
		if err != nil {
	...
	err = validateDiamcircleAsset(cb.Asset)
	if err != nil {
