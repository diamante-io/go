...
# Hello, World!
As is tradition, we'll start with a simplistic example that ingests a single ledger from the network. We're immediately faced with a decision, though: _What's the backend?_ We'll use a **Captive Diamcircle-Core backend** in this example because it requires (little-to-)no setup, but there are couple of alternatives available. You could also use:

  - a **database** (via `NewDatabaseBackend()`), which would ingest ledgers stored in a Diamcircle-Core database, or
...
import (
	...
	backends "github.com/diamcircle/go/ingest/ledgerbackend"
)
...
config := ledgerbackend.CaptiveCoreConfig{
	...
	BinaryPath:         "/usr/bin/diamcircle-core",
	NetworkPassphrase:  networkPassphrase,
...
```
$ go run ./example.go
INFO[...] default: Config from /tmp/captive-diamcircle-core365405852/diamcircle-core.conf  pid=20574
...
In this section, we'll demonstrate how to combine a backend with a reader to actually learn something meaningful about the Diamcircle network. Again, we'll use a specific backend here (Captive Core, again), but the processing can be done with any of them.
...
In this example, we'll leverage the `CheckpointChangeReader` to determine the popularity of a feature introduced in [Protocol 15](https://www.diamcircle.org/blog/protocol-14-improvements): claimable balances. Specifically, we'll be investigating how many claimable balances were created in an arbitrary ledger range.
...
```go
import "github.com/diamcircle/go/historyarchive"
```
...
import (
  ingest "github.com/diamcircle/go/ingest/ledgerbackend"
  "github.com/diamcircle/go/support/log"
  "github.com/sirupsen/logrus"
)
...
```
  ...
  **Please don't use it in production code**; it's provided here for completeness, convenience, and brevity of examples.
  ...
  2. Since the Diamcircle testnet undergoes periodic resets, the example outputs from various sections (especially regarding network statistics) will not always be accurate.
