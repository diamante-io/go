...
// LookupByAddress performs a federated lookup following to the diamcircle
...
// "name" type is a legacy holdover from the legacy diamcircle network's federation
...
// LookupByAccountID performs a federated lookup following to the diamcircle
...
// ForwardRequest performs a federated lookup following to the diamcircle
...
func (c *Client) getFederationServer(domain string) (string, error) {
	stoml, err := c.DiamcircleTOML.GetDiamcircleToml(domain)
	if err != nil {
		return "", errors.Wrap(err, "get diamcircle.toml failed")
	}

	if stoml.FederationServer == "" {
		return "", errors.New("diamcircle.toml is missing federation server info")
	}
