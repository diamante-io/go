...
func TestLiquidityPoolsRequest(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		assert.Equal(t, links.Self.Href, "https://aurora.diamcircle.org/liquidity_pools?limit=200\u0026order=asc")

		assert.Equal(t, links.Next.Href, "https://aurora.diamcircle.org/liquidity_pools?limit=200\u0026order=asc")
...
var liquidityPoolsResponse = `{
  "_links": {
    "self": {
      "href": "https://aurora.diamcircle.org/liquidity_pools?limit=200\u0026order=asc"
    },
    "next": {
      "href": "https://aurora.diamcircle.org/liquidity_pools?limit=200\u0026order=asc"
    }
