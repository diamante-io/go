// Copyright 2016 diamcircle Development Foundation and contributors. Licensed
...
func GetTestS3Archive() *Archive {
	...
	bucket := fmt.Sprintf("s3://history-stg.diamcircle.org/dev/archivist/test-%s", r)
	region := "eu-west-1"
		...
		return ioutil.NopCloser(strings.NewReader(`
{
	...
	assert.NoError(t, err)
	_, err = archive.GetPathHAS("has.json")
	assert.EqualError(t, err, "Network passphrase does not match! expected=Test SDF Network ; September 2015 actual=Public Global diamcircle Network ; September 2015")
}
