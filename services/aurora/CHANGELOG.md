...
* Improve performance of `/trades?trade_type=liquidity_pool` requests. ([4149](https://go/pull/4149))
* Added `absBeforeEpoch` to ClaimableBalance API Resources. It will contain the Unix epoch representation of absolute before date. ([4148](https://go/pull/4148))  
...
### Features
* Result codes for fee-bump transactions will now also include the inner result codes ([4081](https://go/pull/4081))

### Performance improvements
* XDR encoding/decoding pipelines have been optimized ([4069](https://go/pull/4069), [4068](https://go/pull/4068), [4073](https://go/pull/4073), [4064](https://go/pull/4064), [4071](https://go/pull/4071), [4075](https://go/pull/4075), [4077](https://go/pull/4077))

* Path-finding on the `/paths` endpoint has been sped up significantly ([4091](https://go/pull/4091), [4096](https://go/pull/4096), [4102](https://go/pull/4102)), [4105](https://go/pull/4105), [4113](https://go/pull/4113)

* Unused database indices have been removed ([4085](https://go/pull/4085), [4089](https://go/pull/4089))

### Fixes
* Improves error parsing from Captive Core ([4066](https://go/pull/4066))

* Prevent duplicate errors related to liquidity pool tables during repeated reingestion of same range ([4114](https://go/pull/4114))

* In the 2.11.0 release there was a bug introduced which made the `aurora db reingest range` command ignore optional parameters like `--parallel-workers`. This bug is now fixed so all optional command line flags are parsed correctly ([4127](https://go/pull/4127))

## v2.11.0

### Changes

* Add a new aurora flag `--max-assets-per-path-request` (`15` by default) that sets the number of assets to consider for strict-send and strict-recieve requests ([4046](https://go/pull/4046))
* Add an endpoint `/liquidity_pools?account={account_id}` which returns the liquidity pools an account is participating in [4043](https://go/pull/4043)
* Add a new aurora command `aurora db fill-gaps` which fills any gaps in history in the aurora db. The command takes optional start and end ledger parameters. If the start and end ledger is provided then aurora will only fill the gaps found within the given ledger range [4060](https://go/pull/4060)
* Improve performance of `/liquidity_pools/{liquidity_pool_id}/effects` endpoint by optimizing the db query to fetch effects for a liquidity pool [4065](https://go/pull/4065)
* Include the captive core binary in the `diamcircle/aurora` Docker image [4019](https://go/pull/4019)
* Remove `--captive-core-reuse-storage-dir` aurora flag [4048](https://go/pull/4048)
* Improve performance of XDR encoding which should also improve ingestion speeds [4063](https://go/pull/4063), [4056](https://go/pull/4056), [3957](https://go/pull/3957)
* Improve detection of when the diamcircle Core binary has been modified [4050](https://go/pull/4050)
* `aurora_ingest_state_verify_ledger_entries` metric was changed to gauge [4054](https://go/pull/4054)
...
* Use the correct asset when calculating liquidity pool disbursements ([4018](https://go/pull/4018))
* Make sure diamcircle-Core is not started before previous instance termination ([4020](https://go/pull/4020))
* Add a new feature flag `--ingest-enable-extended-log-ledger-stats` (`false` by default) that enables extra ledger stats when logging ledger processing info ([4017](https://go/pull/4017))
* Add a new command `aurora record-metrics` that records `:[ADMIN_PORT]/metrics` into a zip file for debugging purposes ([4023](https://go/pull/4023))
* Expose the `Latest-Ledger` header to browser web pages ([3995](https://go/pull/3995))
* Correct `aurora db reingest range` output command name when invoking `aurora db detect-gaps` ([4007](https://go/pull/4007))
* Add new prometheus metrics:
  * `round_trip_time_seconds`:  time required to run `select 1` query in the DB ([4009](https://go/pull/4009))
  * `state_verify_ledger_entries_count`: number of ledger entries downloaded from buckets in a single state verifier run ([4015](https://go/pull/4015))
  * `ledger_fetch_duration_seconds`: duration of fetching ledgers from ledger backend, sliding window = 10m ([4016](https://go/pull/4016))
...
* The `--ingest` flag is set by default. If `--captive-core-config-path` is not set, the config file is generated based on network passhprase ([3783](https://go/pull/3783)).
...
* Update `/paths` endpoint to take liquidity pools into account when searching for possible routes between assets ([3818](https://go/pull/3818)).
* Multiple performance improvements in `/paths`: [3816](https://go/pull/3816), [3965](https://go/pull/3965), [3933](https://go/pull/3933).
* Requests to `/paths` are now cancelled, respecting `--connection-timeout` flag value ([3081](https://go/pull/3081)).
* Multiple performance improvements to state ingestion processors: [3945](https://go/pull/3945), [3956](https://go/pull/3956), [3963](https://go/pull/3963), [3953](https://go/pull/3953), [3944](https://go/pull/3944).
* Add missing tx result codes in `txsub` ([3866](https://go/pull/3866)).
* Add new metric `ProcessorsRunDurationSummary`, old `ProcessorsRunDuration` is deprecated ([3940](https://go/pull/3940)).
* Logs during state ingesiton now display `progress` value which is percentage progress indicator ([3946](https://go/pull/3946)).
...
* Fix ingestion of fee bump transactions which have muxed source accounts ([3948](https://go/pull/3948)).
* Add an index on trade aggregations, to improve ingestion performance ([3947](https://go/pull/3947)).
...
* Improve performance of `OffersProcessor`. This should speed up ingestion of latest diamcircle Public Network activity by up to 30%. Please note that this change does not improve reingestion speed because ledger entries are not processed during reingestion. ([3917](https://go/pull/3917))
...
* Fix bug in asset balance classification where clawback is enabled. ([3847](https://go/pull/3847))
...
* Limit reap to 100k ledgers/second, to prevent excess CPU usage ([3823](https://go/pull/3823)).
* Improve performance of path finding endpoints ([3818](https://go/pull/3818)).
...
* If `--captive-core-config-path` is not set, the config file is generated based on network passhprase. ([3783](https://go/pull/3783))
* Fix bug in aurora reap system (used by `aurora db reap` command and when aurora is configured with `--history-retention-count`) which could lead to partial deletions. ([3754](https://go/pull/3754))
* Log debug messages from captive core at the appropriate log level. ([3746](https://go/pull/3746))
* Add a feature flag `--captive-core-reuse-storage-path`/`CAPTIVE_CORE_REUSE_STORAGE_PATH` that will reuse Captive Core's storage path for bucket files when applicable for better performance. ([3750](https://go/pull/3750))

* Add the ability to filter accounts by their participation in a particular liquidity pool ([3873](https://go/pull/3873)).

### Update
* Include pool shares in account balances ([3873](https://go/pull/3873)).

## v2.6.1

**Upgrading to this version from <= v2.1.1 will trigger a state rebuild. During this process (which will take at least 10 minutes), aurora will not ingest new ledgers.**

* Fix bug introduced in v2.6.0 ([#3737](https://go/pull/3737)), preventing usage of `aurora db migrate up/down/redo` commands. ([#3762](https://go/pull/3762))

## v2.6.0

**Upgrading to this version from <= v2.1.1 will trigger a state rebuild. During this process (which will take at least 10 minutes), aurora will not ingest new ledgers.**

* Precompute trade aggregations during ingestion to improve performance. Will rebuild the aggregations as part of the database migrations. ([3641](https://go/pull/3641) & [3760](https://go/pull/3760)).
* Require `COUNT` param when running `aurora db migrate down COUNT` to prevent accidentally running all downwards migrations. Add `aurora db migrate status` command. ([#3737](https://go/pull/3737))
* Fix a bug in `fee_account_muxed` and `fee_account_muxed_id` fields (the fields were incorrectly populated with the source account details). ([3735](https://go/pull/3735))
* Validate ledger range when calling `aurora db reingest range` so that we respond with an error when attempting to ingest ledgers which are not available in the history archives. ([3738](https://go/pull/3738))
* Improve performance of transaction submission. ([3563](https://go/pull/3563))
...
* Fix a bug in the method unmarshaling payment operation details. ([#3722](https://go/pull/3722))
...
* Add new command `aurora db detect-gaps`, which detects ingestion gaps in the database. The command prints out the `db reingest` commands to run in order to fill the gaps found ([3672](https://go/pull/3672)). 
* Performance improvement: Captive Core now reuses bucket files whenever it finds existing ones in the corresponding `--captive-core-storage-path` (introduced in [v2.1.0](#v2.1.0) rather than generating a one-time temporary sub-directory ([3670](https://go/pull/3670)). **This feature requires diamcircle-Core version 17.1 or later.**
* aurora now monitors the diamcircle Core binary on disk (pointed to by `--diamcircle-core-binary-path`/`diamcircle_CORE_BINARY_PATH`) and restarts its Captive Core subprocess if it detects changes (i.e a more recent file timestamp for the diamcircle Core binary) ([3687](https://go/pull/3687)).
* `POST /transactions` return `503 Service Unavailable` instead of `504 Gateway Timeout` if connected diamcircle-Core is out of sync ([3653](https://go/pull/3653)).
* Add protocol version metrics: `aurora_ingest_max_supported_protocol_version`, `aurora_ingest_captive_diamcircle_core_supported_protocol_version`, `aurora_diamcircle_core_supported_protocol_version` ([3634](https://go/pull/3634)).
* Fixed crash in `aurora ingest verify-range` command ([3682](https://go/pull/3682)).
* Handle replica conflict errors gracefully ([3674](https://go/pull/3674)).
* Fix data race in request parameters handling ([3690](https://go/pull/3690)).
* Fix bug where the configuration for `CAPTIVE_CORE_LOG_PATH`, `CAPTIVE_CORE_PEER_PORT`, and `CAPTIVE_CORE_HTTP_PORT` were ignored if they were configured via environment variables instead of command line arguments. ([3702](https://go/pull/3702)).
* Error when setting `BUCKET_DIR_PATH` through `--captive-core-config-path` ([3707](https://go/pull/3707)).
...
### Bug Fixes
* Fix bug in `aurora db reingest range` command, which would throw a duplicate entry conflict error from the DB. ([3661](https://go/pull/3661)).
* Fix bug in DB metrics preventing aurora from starting when read-only replica middleware is enabled. ([3668](https://go/pull/3668)).
* Fix bug in the value of `route` in the logs for rate-limited requests (previously it was set to `undefined`). ([3658](https://go/pull/3658)).
...
* Deprecate `--captive-core-config-append-path` in favor of `--captive-core-config-path`. The difference between the two flags is that `--captive-core-config-path` will validate the configuration file to reject any fields which are not supported by captive core ([3629](https://go/pull/3629)).

### New features 

* Add more in-depth Prometheus metrics (count & duration) for db queries. ([3597](https://go/pull/3597), [3605](https://go/pull/3605))

* HTTP request logs will now print the Origin header if Referer is not set. ([3599](https://go/pull/3599))
...
* Fix bug in `aurora db reingest range` command which required the `--ingest` flag to be set ([3625](https://go/pull/3625)).

* Fix bug in causing database connections to be closed when the HTTP request was cancelled. ([3630](https://go/pull/3630))
...
### New features
* Introduces a flag (`--ro-database-url` / `RO_DATABASE_URL`) which allows setting a connection to a read-replica database. This flag makes aurora take into account data propagation lag to the replica instance, adding retries if the data is out of sync ([3574](https://go/pull/3574)).

### Code changes
* Improved test suite coverage and stability ([3560](https://go/pull/3560), [3562](https://go/pull/3562), [3551](https://go/pull/3551), and [3547](https://go/pull/3547)).

* Improved session handling and timeouts ([3576](https://go/pull/3576), [3545](https://go/pull/3545), and [3567](https://go/pull/3567)).

* Improved stability of Captive Core's configuration options. Specifically, it will now prefer either the command-line parameter (e.g. `--captive-core-peer-port` or its env-var equivalent) or the user-supplied append file (`--captive-core-append-path`) over aurora's internal defaults. However, if a value is set in *both* the append file and at the command-line, an error will be thrown unless both values are equal ([3558](https://go/pull/3558)).
...
* Add `num_claimable_balances` and `claimable_balances_amount` fields to asset stat summaries at `/assets` ([3502](https://go/pull/3502)).
* Improve ingestion reliability when running multiple aurora ingestion instances ([3518](https://go/pull/3518)).

## v2.1.1

* When ingesting a backlog of ledgers, aurora sometimes consumes ledgers faster than the rate at which Captive Core emits them. Previously this scenario caused failures in the ingestion system. That is now fixed in ([3531](https://go/pull/3531)).
...
* Add a flag `--captive-core-storage-path`/`CAPTIVE_CORE_STORAGE_PATH` that allows users to control the storage location for Captive Core bucket data ([3479](https://go/pull/3479)).
...
* Add a flag `--captive-core-log-path`/`CAPTIVE_CORE_LOG_PATH` that allows users to control the location of the logs emitted by Captive Core ([3472](https://go/pull/3472)). If you have a `LOG_FILE_PATH` entry in your Captive Core toml file remove that entry and use the aurora flag instead.
* `--diamcircle-core-db-url` / `diamcircle_CORE_DATABASE_URL` should only be configured if aurora ingestion is enabled otherwise aurora will not start ([3477](https://go/pull/3477)).

### New features

* Add an endpoint which determines if aurora is healthy enough to receive traffic ([3435](https://go/pull/3435)).
* Sanitize route regular expressions for Prometheus metrics ([3459](https://go/pull/3459)).
* Add asset stat summaries per trust-line flag category ([3454](https://go/pull/3454)).
...
* Add a flag `--captive-core-peer-port`/`CAPTIVE_CORE_PEER_PORT` that allows users to control which port the Captive Core subprocess will bind to for connecting to the diamcircle swarm. ([3483](https://go/pull/3484)).
...
Please read the [Captive Core](https://go/blob/master/services/aurora/internal/docs/captive_core.md) doc which contains new requirements and migration guide.
...
If you run into issues please check [Known Issues](https://go/blob/master/services/aurora/internal/docs/captive_core.md#known-issues) or [report an issue](https://go/issues/new/choose). Please ask questions in [Keybase](https://keybase.io/team/diamcircle.public) or [Stack Exchange](https://diamcircle.stackexchange.com/).

### Breaking changes

* There are new config params (below) required by Captive diamcircle-Core. Please check the [Captive Core](https://go/blob/master/services/aurora/internal/docs/captive_core.md) guide for migration tips.
  * `diamcircle_CORE_BINARY_PATH` - a path to diamcircle-Core binary,
...
Please read the [Captive Core](https://go/blob/release-aurora-v2.0.0-beta/services/aurora/internal/docs/captive_core.md) doc which contains new requirements and migration guide.
...
If you run into issues please check [Known Issues](https://go/blob/release-aurora-v2.0.0-beta/services/aurora/internal/docs/captive_core.md#known-issues) or [report an issue](https://go/issues/new/choose). Please ask questions in [Keybase](https://keybase.io/team/diamcircle.public) or [Stack Exchange](https://diamcircle.stackexchange.com/).
...
* Fix bug `/fee_stats` endpoint. The endpoint was not including the additional base fee charge for fee bump transactions ([#3354](https://go/pull/3354))
* Expose the timestamp of the most recently ingested ledger in the root resource response and in the `/metrics` response ([#3281](https://go/pull/3281))
* Add `--checkpoint-frequency` flag to configure how many ledgers span a history archive checkpoint ([#3273](https://go/pull/3273)). This is useful in the context of creating standalone diamcircle networks in [integration tests](internal/docs/captive_core.md#private-networks).
...
* Fixed a bug in `/fee_stats` endpoint that could calculate invalid stats if fee bump transactions were included in the ledger ([#3326](https://go/pull/3326))
...
* Improved performance of `OfferProcessor` ([#3249](https://go/pull/3249)).
* Improved speed of state verification startup time ([#3251](https://go/pull/3251)).
* Multiple Captive Core improvements and fixes ([#3237](https://go/pull/3237), [#3257](https://go/pull/3257), [#3260](https://go/pull/3260), [#3264](https://go/pull/3264), [#3262](https://go/pull/3262), [#3265](https://go/pull/3265), [#3269](https://go/pull/3269), [#3271](https://go/pull/3271), [#3270](https://go/pull/3270), [#3272](https://go/pull/3272)).

## v1.12.0

* Add Prometheus metrics for the duration of ingestion processors ([#3224](https://go/pull/3224))
* Many Captive Core improvements and fixes ([#3232](https://go/pull/3232), [#3223](https://go/pull/3223), [#3226](https://go/pull/3226), [#3203](https://go/pull/3203), [#3189](https://go/pull/3189),  [#3187](https://go/pull/3187))

## v1.11.1

* Fix bug in parsing `db-url` parameter in `aurora db migrate` and `aurora db init` commands ([#3192](https://go/pull/3192)).

## v1.11.0

* The `service` field emitted in ingestion logs has been changed from `expingest` to `ingest` ([#3118](https://go/pull/3118)).
* Ledger stats are now exported in `/metrics` in `aurora_ingest_ledger_stats_total` metric ([#3148](https://go/pull/3148)).
* diamcircle Core database URL is no longer required when running in captive mode ([#3150](https://go/pull/3150)).
* xdr: Add a custom marshaller for claim predicate timestamp  ([#3183](https://go/pull/3183)).
...
* Fixed a bug that caused a fresh instance of aurora to be unable to sync with testnet (Protocol 14) correctly. ([#3100](https://go/pull/3100))
* Add Golang- and process-related metrics. ([#3103](https://go/pull/3103))
* New `network_passphrase` field in History Archives (added in diamcircle-Core 14.1.0) is now checked. aurora will return error if incorrect archive is used. ([#3082](https://go/pull/3082))
* Fixed a bug that caused some errors to be logged with `info` level instead of `error` level. ([#3094](https://go/pull/3094))
* Fixed a bug in `/claimable_balances` that returned 500 error instead of 400 for some requests. ([#3088](https://go/pull/3088))
* Print a friendly message when aurora does not support the current diamcircle protocol version. ([#3093](https://go/pull/3093))

## v1.9.1

* Fixed a bug that caused a fresh instance of aurora to be unable to sync with testnet (Protocol 14) correctly. ([#3096](https://go/pull/3096))
* Use underscore in JSON fields for claim predicate to make the API consistent. ([#3086](https://go/pull/3086))
...
* Fixed a data race when shutting down the HTTP server. ([#2958](https://go/pull/2958)).
* Fixed emitting incorrect errors related to OrderBook Stream when shutting down the app. ([#2964](https://go/pull/2964))
...
To solve this it's now possible to start a [`captivecore`](https://go/tree/master/exp/services/captivecore) on another machine and configure aurora to use it in ingestion. This requires two config options set:
...
* Fix panic on missing command line arguments [#2872](https://go/pull/2872)
* Fix race condition where submitting a transaction to aurora can result in a bad sequence error even though diamcircle Core accepted the transaction. [#2877](https://go/pull/2877)
* Add new DB metrics ([#2844](https://go/pull/2844)):
...
* Add `--parallel-workers` and `--parallel-job-size` to `aurora db reingest range`. `--parallel-workers` will parallelize reingestion using the supplied number of workers. ([#2724](https://go/pull/2724))
* Remove diamcircle Core's database dependency for non-ingesting instances of aurora.  ([#2759](https://go/pull/2759))
...
* HAL response population is implemented using Go `strings` package instead of `regexp`, improving its performance. ([#2806](https://go/pull/2806))
* Fix a bug in `POST /transactions` that could cause `tx_bad_seq` errors instead of processing a valid transaction. ([#2805](https://go/pull/2805))
* The `--connection-timeout` param is ignored in `POST /transactions`. The requests sent to that endpoint will always timeout after 30 seconds. ([#2818](https://go/pull/2818))
...
* Add experimental support for live ingestion using a diamcircle Core subprocess instead of a persistent diamcircle Core database.
  ...
  - `--diamcircle-core-binary-path` or `diamcircle_CORE_BINARY_PATH`.

## v1.5.0

### Changes

* Remove `--ingest-failed-transactions` flag. From now on aurora will always ingest failed transactions. WARNING: If your application is using aurora DB directly (not recommended!) remember that now it will also contain failed txs. ([#2702](https://go/pull/2702)).
* Add transaction set operation count to `history_ledger`([#2690](https://go/pull/2690)).
...
* Fix `/metrics` end-point ([#2717](https://go/pull/2717)).
* Gracefully handle incorrect assets in the query parameters of GET `/offers` ([#2634](https://go/pull/2634)).
* Fix logging message in OrderBookStream ([#2699](https://go/pull/2699)).
* Fix data race in root endpoint ([#2745](https://go/pull/2745)).

### Experimental

* Add experimental support for database reingestion using a diamcircle Core subprocess instead of a persistent diamcircle Core database ([#2695](https://go/pull/2695)).
...
- Add the new field `tx_set_operation_count` to the `ledger` resource ([#2690](https://go/pull/2690)). This field can be a `number` or `null`.
...
* Decreased a memory usage of initial state ingestion stage and state verifier ([#2618](https://go/pull/2618)).
* Remove `--exp-ingest-in-memory-only` aurora flag. The in memory order book graph which powers the path finding endpoints is now updated using the aurora DB instead of directly via ingestion ([#2630](https://go/pull/2630)).

## v1.3.0

### Breaking changes

* The type for the following attributes has been changed from `int64` to `string` ([#2555](https://go/pull/2555)):
...
* Add `last_modified_time` to account responses. `last_modified_time` is the closing time of the most recent ledger in which the account was modified ([#2528](https://go/pull/2528)).
* Balances in the Account resource are now sorted by asset code and asset issuer ([#2516](https://go/pull/2516)).
* Ingestion system has its dedicated DB connection pool ([#2560](https://go/pull/2560)).
* A new metric has been added to `/metrics` ([#2537](https://go/pull/2537) and [#2553](https://go/pull/2553)):
...
* Fix a memory leak in the code responsible for streaming ([#2548](https://go/pull/2548), [#2575](https://go/pull/2575) and [#2576](https://go/pull/2576)).
...
* Remove `id` columns from `history_operation_participants` and `history_transaction_participants` to prevent possible integer overflow [#2532](https://go/pull/2532).
## v1.2.0
...
* Added support for [CAP-27](https://github.com/diamcircle/diamcircle-protocol/blob/master/core/cap-0027.md) and [SEP-23](https://github.com/diamcircle/diamcircle-protocol/blob/master/ecosystem/sep-0023.md) [#2491](https://go/pull/2491).
...
`memo_bytes` stores the base 64 encoding of the memo bytes set in the transaction envelope [#2485](https://go/pull/2485).
...
### Changes
* DB: Remove unnecessary duplicate indexes: `index_history_transactions_on_id`, `index_history_ledgers_on_id`, `exp_asset_stats_by_code`, and `asset_by_code` ([#2419](https://go/pull/2419)).
* DB: Remove asset_stats table which is no longer necessary ([#2419](https://go/pull/2419)).
* Validate transaction hash IDs as 64 lowercase hex chars. As such, wrongly-formatted parameters which used to cause 404 (`Not found`) errors will now cause 400 (`Bad request`) HTTP errors ([#2394](https://go/pull/2394)).
* Fix ask and bid price levels of `GET /order_book` when encountering non-canonical price values. The `limit` parameter is now respected and levels are coallesced properly. Also, `price_r` is now in canonical form ([#2400](https://go/pull/2400)).
* Added missing top-level HAL links to the `GET /` response ([#2407](https://go/pull/2407)).
* Full transaction details are now included in the `POST /transactions` response. If you submit a transaction and it succeeds, the response will match the `GET /transactions/{hash}` response ([#2406](https://go/pull/2406)).
* The following attributes are now included in the transaction resource:
    ...
    * `inner_transaction` (only present in Protocol 13 fee bump transactions) ([#2406](https://go/pull/2406)).
* Add support for [CAP0018](https://github.com/diamcircle/diamcircle-protocol/blob/master/core/cap-0018.md): Fine-Grained Control of Authorization (Protocol 13) ([#2423](https://go/pull/2423)).
...
* It is no longer possible to use Redis as a mechanism for rate-limiting requests ([#2409](https://go/pull/2409)).
...
### Fixed
* Fix `aurora db reap` bug which caused the command to exit without deleting any history table rows ([#2336](https://go/pull/2336)).
* The aurora reap system now also deletes rows from `history_trades`. Previously, the reap system only deleted rows from `history_operation_participants`, `history_operations`, `history_transaction_participants`, `history_transactions`, `history_ledgers`, and `history_effects` ([#2336](https://go/pull/2336)).
* Fix deadlock when running `aurora db reingest range` ([#2373](https://go/pull/2373)).
* Fix signer update effects ([#2375](https://go/pull/2375)).
* Fix incorrect error in log when shutting down the system while `verifyState` is running ([#2366](https://go/pull/2366)).
* Expose date header to CORS clients ([#2316](https://go/pull/2316)).
* Fix inconsistent ledger view in `/accounts/{id}` when streaming ([#2344](https://go/pull/2344)).

### Removed
* Dropped support for Go 1.12. ([#2346](https://go/pull/2346)).
...
The [admin guide](https://go/blob/release-aurora-v0.25.0/services/aurora/internal/docs/admin.md) contains all the information needed to operate the new ingestion system.
...
- Change multiple operation types to their canonical names for [operation resources](https://www.diamcircle.org/developers/aurora/reference/resources/operation.html) ([#2134](https://go/pull/2134)).
- Change the type of the following fields from `number` to `string`:
    ...
    See [#1609](https://go/issues/1609), [#1909](https://go/pull/1909) and [#1912](https://go/issues/1912) for more details.
...
- Remove `fee_paid` field from [Transaction resource](https://www.diamcircle.org/developers/aurora/reference/resources/transaction.html) (Use `fee_charged` and `max_fee` fields instead - see [#1372](https://go/issues/1372)).

## v0.24.1

* Add cache to improve performance of experimental ingestion system (#[2004](https://go/pull/2004)).
* Fix experimental ingestion bug where ledger changes were not applied in the correct order (#[2050](https://go/pull/2050)).
* Fix experimental ingestion bug where unique constraint errors are incurred when the ingestion system has to reingest state from history archive checkpoints (#[2055](https://go/pull/2055)).
* Fix experimental ingestion bug where a race condition during shutdown leads to a crash (#[2058](https://go/pull/2058)).

## v0.24.0

* Add `fee_charged` and `max_fee` objects to `/fee_stats` endpoint ([#1964](https://go/pull/1964)).
* Experimental ledger header ingestion processor ([#1949](https://go/pull/1949)).
* Improved performance of asset stats processor ([#1987](https://go/pull/1987)).
* Provide mechanism for retrying XDR stream errors ([#1899](https://go/pull/1899)).
* Emit error level log after 3 failed attempts to validate state ([#1918](https://go/pull/1918)).
* Fixed out of bounds error in ledger backend reader ([#1914](https://go/pull/1914)).
* Fixed out of bounds error in URL params handler ([#1973](https://go/pull/1973)).
* Rename `OperationFeeStats` to `FeeStats` ([#1952](https://go/pull/1952)).
* All DB queries are now cancelled when request is cancelled/timeout. ([#1950](https://go/pull/1950)).
...
Check [Beta Testing New Ingestion System](https://go/blob/master/services/aurora/internal/expingest/BETA_TESTING.md) if you want to test the new ingestion system.
...
* New features in experimental ingestion (to enable: set `--enable-experimental-ingestion` CLI param or `ENABLE_EXPERIMENTAL_INGESTION=true` env variable):
  ...
  * `/order_book` offers data is served from in-memory store ([#1761](https://go/pull/1761))

  * Add `Latest-Ledger` header with the sequence number of the most recent ledger processed by the experimental ingestion system. Endpoints built on the experimental ingestion system will always respond with data which is consistent with the ledger in `Latest-Ledger` ([#1830](https://go/pull/1830))
  ...
  * Add experimental support for filtering accounts who are trustees to an asset via `/accounts`. Example:\
  ...
  returns all accounts who have a trustline to the asset `COP` issued by account `GC2GFG...` ([#1835](https://go/pull/1835))

  * Experimental "Accounts For Signers" end-point now returns a full account resource ([#1876](https://go/issues/1875))
* Prevent "`multiple response.WriteHeader calls`" errors when streaming ([#1870](https://go/issues/1870))
* Fix an interpolation bug in `/fee_stats` ([#1857](https://go/pull/1857))
* Fix a bug in `/paths/strict-send` where occasionally bad paths were returned ([#1863](https://go/pull/1863))
...
Check [Beta Testing New Ingestion System](https://go/blob/master/services/aurora/internal/expingest/BETA_TESTING.md) if you want to test new ingestion system.
...
* Path finding bugs have been fixed and the algorithm has been improved. Check [#719](https://go/pull/719) for more information.
...
- Fixed an issue caused by un-migrated trade rows. (https://go/issues/357)
...
- Upstream bug fixes to xdr decoding from `go`.
